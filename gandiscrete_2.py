...
def main():
    ...
    training_set = original.iloc[0:slpt, 0:6].values.tolist()
    test_set = original.iloc[slpt:original.shape[0], 0:6].values.tolist()
    ...
    with tf.Session() as sess:
        ...
        with open('data/test3_real.csv', 'wb') as f:
            f.write(b'A,B,C,D,E,F\n')
            np.savetxt('data/test3_real.csv', np.array(test_set), delimiter=',', fmt='%.3f')
        ...
        with open('data/test3.csv', 'wb') as f:
            f.write(b'A,B,C,D,E,F\n')
            np.savetxt('data/test3.csv', g[0], delimiter=',', fmt='%.3f')
