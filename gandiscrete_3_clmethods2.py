...
#targetidxlist the indexes to be removed

def gan_org(original,splitpoint, targetidxlist, outputfilename='data/test2.csv'):
    ...
    # targetcolumns = [ training_set[:,i] for i in targetidxlist]
    ...
    weights = {
        'gen_hidden1': tf.Variable(glorot_init([ginput, gen_hidden_dim1])),
        ...
        'gen_out': tf.Variable(glorot_init([gen_hidden_dim3, ginput])),
    ...
    biases = {
        ...
        'gen_out': tf.Variable(tf.zeros([goutput])),
...
def main():
    original = pd.read_csv('data/test4.csv', float_precision='%.3f')
    # original = pd.read_csv('data/adult.data', float_precision='%.3f')
    gan_org(original, splitpoint = 0.9, targetidxlist=[4,5])
